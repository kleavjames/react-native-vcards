...
(function vCardFormatter() {
    ...
    module.exports = {
        ...
        getFormattedString: function(vCard) {
            ...
            if (vCard.workEmail) {
                formattedVCardString += 'EMAIL' + ';type=email:' + e(vCard.workEmail) + nl();
                // if (majorVersion >= 4) {
                //     formattedVCardString += 'EMAIL' + ';CHARSET=UTF-8' + ';type=email:' + e(vCard.workEmail) + nl();
                // } else if (majorVersion >= 3 && majorVersion < 4) {
            ...
            if (vCard.cellPhone) {
                formattedVCardString += 'TEL;TYPE=CELL,VOICE:' + e(vCard.cellPhone) + nl();
            }
            ...
            if (vCard.title) {
                formattedVCardString += 'TITLE' + ';CHARSET=UTF-8' + ':' + e(vCard.title) + nl();
            }

            if (vCard.role) {
                formattedVCardString += 'ROLE' + ';CHARSET=UTF-8' + ':' + e(vCard.role) + nl();
            }

            if (vCard.organization) {
                formattedVCardString += 'ORG' + ';CHARSET=UTF-8' + ':' + e(vCard.organization) + nl();
            }

            if (vCard.url) {
                formattedVCardString += 'URL;type=url' + ';CHARSET=UTF-8' + ':' + e(vCard.url) + nl();
            }

            if (vCard.workUrl) {
                formattedVCardString += 'URL;type=website' + ';CHARSET=UTF-8' + ':' + e(vCard.workUrl) + nl();
            }
